...
export default class PrintDialog extends Component<Props, State> {
  ...
  onConfirmCorrectPrinting = () => {
    this.setState(state => ({ isPrintedCorrectly: !state.isPrintedCorrectly }));
  };

  onConfirmReadable = () => {
    this.setState(state => ({ isReadable: !state.isReadable }));
  };

  onConfirmScannable = () => {
    this.setState(state => ({ isScannable: !state.isScannable }));
  };
}
